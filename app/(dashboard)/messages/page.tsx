  ...
  return (
    <div className="flex h-screen bg-slate-950 overflow-hidden text-slate-100">
      ...
      <main className="flex-1 md:ml-24 flex h-full transition-all duration-500">
        ...
        <div className="hidden md:block w-80 lg:w-96 h-full border-r border-white/5">
          {convsLoading ? (
            <div className="flex items-center justify-center h-full bg-slate-950/50 backdrop-blur-xl">
        ...
        <div className="flex-1 flex flex-col bg-slate-900/10 relative">
          ...
          {activeConversationId && activeConv ? (
            <>
              {/* Chat Header */}
              <header className="h-24 flex items-center justify-between px-8 border-b border-white/5 bg-slate-950/40 backdrop-blur-2xl sticky top-0 z-10">
                <div className="flex items-center gap-4">
                  {otherParticipant ? (
                    ...
                    >
                      <img
                        ...
                        className="w-12 h-12 rounded-2xl object-cover ring-2 ring-slate-800 shadow-xl"
                      />
                      {otherParticipant.status === 'online' && (
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-brand-success border-4 border-slate-950 rounded-full shadow-lg" />
                      )}
                    </motion.div>
                  ) : (
                  ...
                  <div>
                    <h2 className="font-black text-slate-100 text-lg tracking-tight flex items-center gap-2">
                ...
                <div className="flex items-center gap-3">
                  <div className="hidden sm:flex items-center gap-2">
                    <button className="p-3 bg-slate-900/50 border border-white/5 rounded-2xl text-slate-400 hover:text-brand-primary hover:bg-brand-primary/10 transition-all active:scale-90">
                      <Phone size={20} />
                    </button>
                    <button className="p-3 bg-slate-900/50 border border-white/5 rounded-2xl text-slate-400 hover:text-brand-primary hover:bg-brand-primary/10 transition-all active:scale-90">
                      <Video size={20} />
                  ...
                  <button className="p-3 bg-slate-900/50 border border-white/5 rounded-2xl text-slate-400 hover:text-white transition-all active:scale-90">
                    <Info size={20} />
                  </button>
                </div>
              </header>
              ...
              <div className="flex-1 overflow-y-auto p-8 no-scrollbar flex flex-col gap-6 relative z-0">
                {msgsLoading ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center max-w-sm mx-auto">
                    ...
                    <h3 className="text-xl font-black text-slate-100 mb-2 tracking-tight">The start of something new!</h3>
              ...
              {/* Input Area */}
              <footer className="p-8 bg-slate-950/40 backdrop-blur-2xl border-t border-white/5 sticky bottom-0 z-10">
                <form
                  onSubmit={handleSendMessage}
                  className="max-w-5xl mx-auto flex items-center gap-4"
                >
                  <button type="button" className="p-4 bg-slate-900/80 border border-white/5 rounded-[1.5rem] text-slate-500 hover:text-brand-primary hover:bg-brand-primary/5 transition-all shadow-xl active:scale-90">
                    <Plus size={20} />
                  </button>
                  ...
                  <div className="flex-1 relative group">
                    ...
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-3">
                      <button type="button" className="text-slate-600 hover:text-brand-accent transition-colors">
                        <Smile size={20} />
                      </button>
                      <button type="button" className="text-slate-600 hover:text-brand-primary transition-colors">
                        <Paperclip size={20} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
              <div className="w-32 h-32 bg-slate-900 rounded-[3rem] border border-white/5 flex items-center justify-center mb-8 text-slate-800 relative group overflow-hidden">
              ...
              <h2 className="text-3xl font-black text-slate-100 mb-3 tracking-tighter">Campus Chat</h2>
              <p className="text-slate-500 text-lg font-medium max-w-md">Select a teammate or activity group to start the conversation.</p>
              <button className="btn-primary mt-10 px-10 py-4 text-xs font-black uppercase tracking-widest">
                Start New Chat
              </button>
            </div>
          )}
