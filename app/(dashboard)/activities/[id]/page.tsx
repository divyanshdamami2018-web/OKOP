  ...
  if (!activity) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-100">
        ...
        <button onClick={() => router.push('/feed')} className="text-blue-500 hover:underline">Return to feed</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex">
      ...
      <main className="flex-1 ml-20">
        <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl border-b border-slate-900 px-8 py-4">
        ...
        <div className="max-w-4xl mx-auto px-8 py-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                  ...
                  <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold truncate">{activity.location}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activity.tags.map((tag) => (
                      <span key={tag} className="px-4 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-slate-300 font-medium">
                        #{tag}
                      </span>
                    ))}
                  </div>
              ...
              <section className="bg-slate-900/30 border border-slate-800/50 rounded-3xl overflow-hidden">
                ...
                <form onSubmit={handleSendMessage} className="p-4 bg-slate-950/50 border-t border-slate-800/50 flex gap-2">
                  <input
                    type="text"
                    value={chatInput}
                    onChange={(e) => setChatInput(e.target.value)}
                    placeholder="Type a message..."
                    className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-blue-500/50"
                  />
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                ...
                <div className="flex items-center gap-4 mb-6">
                  <img src={activity.creator.avatar} className="w-14 h-14 rounded-2xl object-cover ring-2 ring-slate-800" />
                  <div>
                    <h4 className="font-bold text-slate-100">{activity.creator.name}</h4>
                    <p className="text-xs text-slate-500">Student @ {activity.creator.college}</p>
                  </div>
                </div>
                <button
                  onClick={() => router.push(`/profile/${activity.creator.id}`)}
                  className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-sm font-bold transition-all"
                >
                  View Profile
                </button>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-xs font-bold text-blue-500">{activity.currentParticipants}/{activity.maxParticipants}</span>
                </div>
                <div className="flex flex-wrap gap-2 mb-8">
                  {participants.map((p) => (
                    <img
                      ...
                      src={p.avatar}
                      title={p.name}
                      className="w-10 h-10 rounded-xl object-cover ring-2 ring-slate-950 hover:scale-110 transition-transform cursor-pointer"
                    />
                  ))}
                  {[...Array(Math.max(0, activity.maxParticipants - participants.length))].map((_, i) => (
                    <div key={i} className="w-10 h-10 rounded-xl border border-dashed border-slate-800 flex items-center justify-center text-slate-700">
                      <Plus size={16} />
                    </div>
                  ))}
                </div>
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-sm shadow-xl transition-all active:scale-95 flex items-center justify-center gap-2 ${
                    isJoined
                      ? 'bg-slate-800 text-red-400 hover:bg-red-500/10 hover:text-red-400 border border-transparent hover:border-red-500/20'
